...
/**
 * Generate a comprehensive summary of conversation messages with custom user guidance
 * Used by summary nodes when user wants to emphasize specific aspects
 */
export async function generatePathSummaryWithGuidance(
  config: LLMConfig,
  messages: Message[],
  userGuidance?: string
): Promise<string> {
  ...
  if (!endpoint || !model || messages.length === 0) {
    console.warn('generatePathSummaryWithGuidance: Missing required config or no messages', {
      hasEndpoint: !!endpoint,
      hasModel: !!model,
      messageCount: messages.length
    });
  ...
  // Build prompt with optional user guidance
  const basePrompt = `You are creating a detailed summary that will replace the full conversation history for future messages in this thread. This summary is critical - any information you omit will be lost forever.
...
- Any remaining questions or areas to explore further`;
  ...
  const userGuidanceSection = userGuidance && userGuidance.trim()
    ? `

USER'S SPECIFIC GUIDANCE:
The user has requested you pay special attention to the following:
${userGuidance.trim()}

Please incorporate these priorities into your comprehensive summary while still covering all the important aspects listed above.`
    : '';

  const summaryPrompt = {
    role: 'user',
    content: `${basePrompt}${userGuidanceSection}
...
DETAILED SUMMARY:`,
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('generatePathSummaryWithGuidance: LLM request failed', {
        status: response.status,
        statusText: response.statusText,
        error: errorText
      });
    ...
    // Handle multiple response formats (same as generatePathSummary)
    let summary: string | undefined;
    const message = data.choices?.[0]?.message;
    ...
    console.warn('generatePathSummaryWithGuidance: Could not extract summary from response', { data });
    return getFallbackSummary();
  } catch (error) {
    console.error('generatePathSummaryWithGuidance: Exception occurred', error);
    return getFallbackSummary();
  }
}
